...
public class PhysicsCalculator
{
    ...
    //TODO При 5000, 500, 0.05 и 1 получаем MaxSpeed = 300. Надо найти значения Force и NewMass
    //TODO для диапазона скоростей 150-450.
    ...
    //TODO Мб всё таки сделать костыль который при превышении макс скорости режет управление
    //TODO на 50-90% ? Типа ты не касаешься земли.
    //TODO Или удаляет GroundFriction совсем. А в обычной ситуации завысить его.
    //TODO Можно через функцию аналогично максимальной скорости рассчитать текущий лаг при
    //TODO развороте, и посчитать для других значений: лаг при развороте, макс скорость,
    //TODO дистанцию отбрасывания, макс. скорость при отбрасывании
    ...
    //TODO Насколько эта функция быстрая? Стоит оптимизировать? Нагрузка на сервер 20-60% TPS,
    //TODO на клиент 3-15% TPS при кол-ве юнитов 10-300
    ...
    //TODO Но для 300 стоячих юнитов нагрузка на сервер тоже выше: 33% против 14%. При этом каждые
    //TODO 2-4 секунды прыжок до 70%. Для 10 юнитов нагрузка одинаковая: 4%.
    ...
    {
        //TODO Из-за того, что клиент и сервер считают физику только чатси юнитов, масса как будто
        //TODO больше не работает при таранах и ничего не делает. Force по прежнему работает.
        //TODO На большой скорости (например, текущие настройки) игрок проскакивает сквозь врага.
        //TODO Скорее всего из-за того, что игрок у себя таранит врага, но на сервере враг таранится
        //TODO медленней и у игрока происходит телепорт врага в старую позицию.
        //TODO Потенциально может помочь увеличение дистанции ТП, но это негативно повлияет на все
        //TODO остальные кейсы рассинхрона.
        //TODO Создать задачу и потестить. Понять что делать с физикой. Мб не нужен Force параметр
        //TODO игроку, а только конечная скорость, которая преобразуется в Force.
        // if (Net.IsClient())
        // {
        //     character.Mass = 10;
    ...
    /// <summary>
    /// Здесь аналитический расчет сил, вместо ApplyCentralForce. Т.к. в ApplyCentralForce сила
    /// применяется на 16/33 мс, и может быть чрезмерна.
    ...
    /// </code>
    /// Также тут были костыли, про то, что при input = Vector.Zero мы применяем engineForce против
    /// движения (помогаем остановиться быстрее).
    ...
    /// В этом коммите обычный метод приложения сил был заменен на аналитический, а также включена
    /// опция custom_integrator, вместо ванильного расчета.
    /// </summary>
    ...
    /// </summary>
    private Vector2 CalculateTerminalVelocity(
        Vector2 input,
        float force,
        float mass,
        float groundFriction,
