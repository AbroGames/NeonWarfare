...
> `--fast-test 2` запускает сервер и 2 клиента, клиенты автоматически подключается к этому серверу.  
> `--auto-connect-ip 127.0.0.1` запускает клиент и автоматически подключается к серверу по адресу указанному адресу (в данном случае 127.0.0.1).  
