...
using NeonWarfare.Scenes.World.Entities.Actions.SlowShot;
...
public class SlowShotSkill : Skill
{
    public SlowShotSkill() : base(SkillTypeConst)
    {
    ...
    public const string SkillTypeConst = "SlowShot";
    
    private const ActionInfoStorage.ActionType ActionType = ActionInfoStorage.ActionType.SlowShot;
    private const double Speed = 800;
    private const double Range = 1000;
    private const double Slow = 0.5;
    ...
    {
        ServerSlowShotAction shotAction = useInfo.World.CreateNetworkEntity<ServerSlowShotAction>(ActionInfoStorage.GetServerScene(ActionType));
        ...
        shotAction.InitStats(
            slow: Slow/useInfo.DamageFactor, 
    ...
    public override void OnClientUse(ClientSkillUseInfo useInfo)
    {
        ...
        ClientSlowShotAction shotAction = useInfo.World.CreateNetworkEntity<ClientSlowShotAction>(ActionInfoStorage.GetClientScene(ActionType), useInfo.Nid);
        shotAction.Init(useInfo.CharacterPosition, useInfo.CharacterRotation);
