...
{
    [GamePacket]
    public class SC_InertiaEntityPacket(long nid, Vector2 position, float rotation,
        float movementRotation, float movementSpeed) : BinaryPacket
    {
        ...
        public SC_InertiaEntityPacket(CS_InertiaEntityPacket packet) : 
            this(packet.Nid, packet.Position, packet.Rotation, packet.MovementRotation, packet.MovementSpeed) { }
    }
    
    [GamePacket]
    public class CS_InertiaEntityPacket(long nid, Vector2 position, float rotation,
        float movementRotation, float movementSpeed) : BinaryPacket
    {
        public override MultiplayerPeer.TransferModeEnum Mode => MultiplayerPeer.TransferModeEnum.Unreliable;
