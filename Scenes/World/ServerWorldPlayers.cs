...
public partial class ServerWorld 
{
    ...
    public void AddEntity(ServerEnemy enemy)
    {
        ...
        //Надо бы иметь общий пул всех объектов (для синхронизации, например). И дополнительные пересекающиеся пулы characters, игроков, врагов и т.п.
        //Но ведь уже есть GetChild<> ??? Может пулы вообще не нужны? Достаточно какого-нибудь автовычисляемого свойства ? Маппинг по nid уже есть, маппинг по peerId тоже есть 
        AddChild(enemy);
    }
